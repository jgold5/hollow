...
pub const Ctx = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Ctx {
        return .{ .allocator = allocator, .cwd = std.fs.cwd(), .env = try std.process.getEnvMap(allocator), .log = std.io.getStdErr(), .project_root = null, .out_dir = null, .config = BuildConfig{ .base_url = "" }, .port = 8000 };
    }
